...
_REFERENCE_PROMPT = (
    ...
    "- CRITICAL TEXT HIERARCHY: Establish clear visual hierarchy via font sizes. Layer titles MUST be the largest and boldest; sub-module titles MUST be medium; descriptive text and lists MUST be standard.\n"
    "- CRITICAL SPACE: Without deleting core keywords, if the content is extensive, appropriately widen the canvas aspect ratio to ensure ALL content is displayed in a SINGLE horizontal line or a perfectly aligned clean sequence. The image MUST NOT be overcrowded; provide appropriate generous whitespace and breathing room around and inside the blocks. Ensure zero text-wrapping.\n"
    "- CRITICAL CONTENT LAYOUT: Within each block, organize sub-elements with a strong sense of symmetry.\n"
    "- CRITICAL ICONOGRAPHY: You MUST incorporate elegant, minimalist, and highly technically-relevant ICONS. Place icons immediately adjacent to their corresponding text labels, ensuring perfect vertical centering between the icon and the text block.\n"
    "- CRITICAL STYLE: Extract the core style/colors from the reference image, but elevate it to a PREMIUM, GORGEOUS, and HIGH-END academic infographic style (e.g., Nature/Science journal quality). Use refined aesthetics (e.g., high-quality vector finish, subtle elegant shading) while strictly maintaining a UNIFIED and COHESIVE visual style. Use a consistent color palette (e.g., monochromatic or strict two-tone) for blocks of the same logical level. DO NOT use random, chaotic, or 'rainbow' colors. Ensure a pure white background and perfectly straight orthogonal lines.\n"
    "- CRITICAL LOGIC: The direction of arrows must strictly represent the causal or data flow described in the text. "
)
